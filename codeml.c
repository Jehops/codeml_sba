...
#define NCATG         40
#define NBTYPE        17
#define MAXSF         100 /* jrm: field size used for SBA smoothing */
...
struct common_info {
  ...
  /* pomega & pkappa are used to communicate between SetParameters & ConditionalPNode 
   ...
   double h; /* smoothing bandwidth parameter */
}  com;
struct TREEB {
...
/* random deviates for SBA */
double runif[2][MAXSF];
...
int GetOptions (char *ctlf)
{
   ...
   for (;;) {
      ...
      for (iopt=0; iopt<nopt; iopt++) {
         if (strncmp(opt, optstr[iopt], 8)==0)  {
	       ...
	       case (37): com.sba=(int)t;         break;
	       case (38): com.h=t;                break;
           }
           break;
         }
      }
   ...
   if (com.seqtype==AAseq || com.seqtype==CODON2AAseq) {
      ...
      if((com.sba) && !(com.NSsites == NSpselection || com.NSsites == NSbetaw))
	  error2("SBA is only implemented for NSsites=2,8");
   ...
   */
   case(NSfreqs): 
	  ...
	  if ( !(com.sba == 2) ) // jrm: not transforming when parameters are fixed
	      ...
	      for(i=0,tot=0; i<K-1; i++) tot += (com.freqK[i] = (x+k)[i]);
	      com.freqK[i] = 1 - tot;
	  }
	  k += K-1;
      }
